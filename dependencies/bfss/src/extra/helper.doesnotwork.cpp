...
void initializeCompose(Aig_Man_t* SAig, vector<Aig_Obj_t* >& Fs,
	...
	for(int i = 0; i < numX; i++) {
		nodeIdtoN[varsXS[i] - 1] = -1;
		nodeIdtoN[numOrigInputs + varsXS[i] - 1] = -1;
		//cout << "Assigning " << i << " and " << numOrigInputs + i << " to X " << endl;
	}
    int nonUnates = 0;
	for(int i = 0; i < numY; i++) {
        if (unate[i] == -1)
		    nodeIdtoN[varsYS[i] - 1] = nonUnates ++;
        else    
		    nodeIdtoN[varsYS[i] - 1] = -2;
		cout << "Assigning " << nodeIdtoN[varsYS[i] - 1] <<  " to Y " << varsYS[i] -1 << endl;
	//	nodeIdtoN[numOrigInputs + varsYS[i] - 1] = numOrigInputs + numX + i;
		//cout << "Assigning " << numX + i << " and " << numOrigInputs + numX + i << " to Y " << endl;
	}
	for(int i = 0; i < numY; i++) {
        if (unate[i] == -1)
		    nodeIdtoN[numOrigInputs + varsYS[i] - 1] = nonUnates ++;
        else    
		    nodeIdtoN[numOrigInputs + varsYS[i] - 1] = -2;
	//	nodeIdtoN[varsYS[i] - 1] = nonUnates ++;
//		nodeIdtoN[numOrigInputs + varsYS[i] - 1] = nonUnates ++;
		cout << "Assigning " << nodeIdtoN[numOrigInputs + varsYS[i] - 1] <<  " to Y " << numOrigInputs + varsYS[i] -1 << endl;
	}

        cout << "# No of CIs in Support " <<  Aig_SupportSize(SAig, Aig_ObjFanin0(Aig_ManCo(SAig, 0)));
	//cout << " NumY = " << numY << endl;
	//Aig_Obj_t * pAigObj;
	//int k;
...
void Aig_ComposeVecVec_rec(Aig_Man_t* p, Aig_Obj_t* pObj, vector<vector<Aig_Obj_t*> >& pFuncVecs) {
	...
	if(Aig_ObjIsCi(pObj)) {
        ...
        if ( i == -2)
            cout << " error - reached unate variable! " << endl;

		cout << "Printing nodeIdtoN @ i " << i << "  " << (pObj -> Id -1) << endl;
