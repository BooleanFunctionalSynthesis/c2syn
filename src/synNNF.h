...
//bool createfromPrep( vector<vector<int> > &clauses, unsigned int nVars, vector<int>& varsY, set<int>& actY);
    void init (bool );
    void CreateSynNNF(vector<vector<int> > &clauses, vector<bool>& tcls, vector<int>& Xvar, vector<int>& Yvar,  int&,  vector<int>&, string, set<int> &, map<int, vector<int> > &, map<int, vector<int> > &, map<int, vector<int> > &, bool solved);
